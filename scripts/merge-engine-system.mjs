/**
 * Bake 010-000 + 107-010 → 引擎系统 GLB（105-020 已拆至 pm-intake）。
 *
 * - Layer TRS from .local/xray-transforms.json
 * - Sub-mesh TRS/visible from .local/xray-mesh-state.json
 * - 107-010 含 tripo_part_2_R
 * - 冲突名：107→_107
 * - Output: .local/petka-models/merged/engine.glb（pm-engine）
 *
 * Usage: node scripts/merge-engine-system.mjs
 */
...
const OUT_ID = "pm-engine";
const OUT_REL = "petka-models/merged/engine.glb";
...
const PARTS = [
  {
    id: "pm-010-000",
    glb: "petka-models/010-000.glb",
    meshStateId: "pm-010-000",
    collideTag: null,
  },
  {
    id: "pm-107-010",
    glb: "petka-models/107-010.glb",
    meshStateId: "pm-107-010",
    collideTag: "_107",
  },
];
...

{
  ...
  if (fs.existsSync(MS_PATH)) {
    fs.copyFileSync(
      MS_PATH,
      path.join(root, ".local", `xray-mesh-state.pre-engine-merge.${stamp}.json`),
    );
  }
  if (fs.existsSync(TF_PATH)) {
    fs.copyFileSync(
      TF_PATH,
      path.join(root, ".local", `xray-transforms.pre-engine-merge.${stamp}.json`),
    );
  }
}
  ...
  JSON.stringify(
    {
      ...
      sample107: Object.keys(outMeshes)
        .filter((n) => n.includes("_107"))
        .slice(0, 8),
    },
    null,
    2,
